...
describe("nexus quote service", () => {
  it("creates and retrieves a quote by quote reference", async () => {
  ...
  it("creates a renewal response from a valid payload", async () => {
    const payload = buildQuotePayload();
    const created = await createQuote(payload);
    const renewed = await renewQuote(created.quote_ref, payload);
  ...
  it("records endorsement for on-cover policies only", async () => {
    ...
    const endorsed = await endorseQuote(created.quote_ref, { note: "Change of address" });
